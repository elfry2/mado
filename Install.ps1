...
# 2. Environment & Configuration Functions
...
function Configure-Yazi {
    Write-Host "`n[*] Configuring Yazi to use Neovim..." -ForegroundColor Cyan
...
"@ | Set-Content -Path "$configDir\yazi.toml" -Encoding utf8
...
function Read-CliCheckboxes {
    ...
    while ($true) {
        ...
        if ($input -match '(?i)^\s*a\s*$') {
            $all = ($selected -notcontains $false)
            for ($i = 0; $i -lt $selected.Count; $i++) { $selected[$i] = -not $all }
        }
...
if ($Targets.Count -eq 0) { Write-Host "No applications selected. Exiting." -ForegroundColor Yellow; Exit }

Clear-Host
foreach ($Target in $Targets) { Install-Application -App $Target }

if ($Targets.Name -contains 'Yazi') { Configure-Yazi }
